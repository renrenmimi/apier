...
const JOURNEY_FRAMES: FlowFrame[] = [
  ...
  {
    stage: <Stage packet="查:42 号用户" packetAt={1} />,
    msg: (
      <>
        服务器转身去问数据库 —— 数据真正住的地方。注意:你的网页
        <b>永远不直接碰数据库</b>,一切都要经过服务器这道关卡。为什么?往下看 §04。
      </>
    ),
  },
  {
    stage: <Stage lit={2} packet="42 号的资料" packetAt={1} back />,
    msg: <>数据库翻出 42 号用户的资料,交还给服务器。</>,
  },
  {
    stage: <Stage lit={1} packet="200 OK + JSON" packetAt={0} back />,
    msg: (
      <>
        服务器把数据打包成 <b>JSON</b>(§05 讲),贴上「<b>200 OK</b>
    ...
    <div className="hm-json">
      <div className="hm-json-win">
        <div className="codewin-bar">
          ...
          <span className="codewin-name">response.json · 点每一行试试</span>
...
export function LiveFetch() {
  ...
  return (
    <div className="viz hm-live">
      <div className="viz-title">真枪实弹:现在就发一个请求</div>
      ...
      {st.phase === "idle" && (
        <div className="viz-msg">
          点一个按钮 —— 你的浏览器会真的向互联网上的公开服务器发一次请求,
          响应原样贴在下面。
        </div>
      )}
      ...
      {st.phase === "done" && (
        <>
          ...
          {st.offline && (
            <div className="viz-msg">
              现在连不上外网,先看一份之前存下来的同款响应 —— 内容格式一模一样。
            </div>
          )}
...
const CAMPS: { id: Chapter["camp"]; label: string }[] = [
  { id: "core", label: "地基 · 人人要过的三关" },
  { id: "rest", label: "REST 蓝方 · 资源的世界" },
  { id: "graphql", label: "GraphQL 粉方 · 按需点菜" },
  { id: "verdict", label: "终章 · 你来做裁判" },
];
