  ...
  },
  {
    ...
    hint: (
      <>
        ...
        <code>new Date(x * 1000)</code> 能把它用直白的话说。
      </>
    ),
    solution: (
      <>
...
console.log(r2.status); // 304 —— 没变,不给正文`}
        />
        <p>
          补充一点:GitHub 的 <b>304 不消耗你的 60 次配额</b> ——
...
export const QUIZ: QuizItem[] = [
  {
    ...
    wrong: [
      ...
      undefined,
      <>
        百万级数据一次全给,服务器、网络、用户手机三处都会被拖垮 ——
        分页就是为了避免这个才存在的。
      </>,
      <>
        用户往下滑是想按顺序看到接下来的内容,结果必须可预期。
        「精确接续」正是分页要保证的底线。
      </>,
    ],
    why: (
  ...
  },
  {
    ...
    why: (
      <>
        ...
        没变就省下正文(304 只回头部),变了给新货(200 + 新 ETag)。
        这四步记牢,协商缓存就通了。
      </>
    ),
  },
